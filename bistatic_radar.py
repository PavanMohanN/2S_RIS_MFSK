...
=============================================================
Implements the bistatic M-FSK radar signal chain for the two-stage
T-RIS / R-RIS system at 28 GHz.

System path (4 hops):
    CW Tx ─(R_TxRIS=8m)→ T-RIS ─(R_RIS_T=10m)→ Target
          ←(R_RIS_T=10m)─ R-RIS ←────────────────
          R-RIS ─(R_T_Rx=8m)→ Rx

Paper equations (Section IV):
    Beat signal   y[k,n]  = A·exp(-j2πkΔfτ)·exp(j2πfd·nTsw) + w  [Eq. 16]
    Range delay   τ        = R_RIS_T / c                            [Eq. 17]
    Doppler freq  fd        = (v·r̂_T + v·r̂_R) / λ                 [Eq. 18]
    Range profile p(r)      = IFFT_k{y[k,n]}                        [Eq. 20]
    R-D map      RD(r,v)    = FFT2{y[k,n]}                          [Eq. 21]

Output figures
--------------
F10 — Link budget waterfall: per-stage gains/losses, single + CPI SNR
F11 — (a) IFFT range profile, (b) Range–Doppler map
      Double-column IEEE TVT (7.16 × 3.0 in)

Output tables
-------------
T1  — System parameters  (tables/T1_system_params.tex + .csv)
T2  — RIS parameters     (tables/T2_ris_params.tex + .csv)

Output data
-----------
data/bistatic_radar_data.npy
...
# ─────────────────────────────────────────────────────────────────────────────
# IEEE TVT publication style (consistent with Phases 1–2)
# ─────────────────────────────────────────────────────────────────────────────
plt.rcParams.update({
    'font.family':          'serif',
...
def compute_link_budget():
    ...
    Key equations (Section IV-B of paper):
        ...
        T-RIS array gain: G_T   = 10log10(N·4π·d²/λ²)   [Eq. 10]
        R-RIS beam gain:  G_R   = 10log10(N²·(d/λ)²·cos²θ₁)  [Eq. 13]
...
def plot_F10(lb, save=True):
    """
    F10 — Cumulative link budget waterfall showing each gain/loss stage.

    Left axis: per-stage contribution (dB bars, positive = gain, negative = loss).
    Right axis: cumulative signal power (dBm, dashed line).
    Horizontal reference line at noise floor (dBm).
    Single-column IEEE TVT (3.5 × 3.4 in).
    """
    stages  = lb['stages']
...
def plot_F11(r_axis, profile_dB, v_axis, RD_dB, meta, lb, peak_r_idx,
             peak_rc, save=True):
    """
    F11 — Two-panel figure:
      (a) IFFT range profile: averaged power vs range, target peak, noise floor
      (b) Range-Doppler map: 2D heatmap with target marker
...
def _tex_table(caption, label, header, rows, notes=''):
    """Build a three-column IEEE-style LaTeX table string."""
    col_fmt = 'l l r'
    lines = [
        r'\begin{table}[!t]',
