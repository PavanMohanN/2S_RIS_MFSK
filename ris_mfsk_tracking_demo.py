...
=========================================================================
Interactive, animated demonstration of the two-stage T-RIS / R-RIS
M-FSK bistatic sensing and beam-tracking system at 28 GHz.

WHAT THIS SHOWS
---------------
A target moves through a bistatic scene exactly as described in the paper
(initial range 10 m at 60 deg, 2 m/s at a 45 deg heading, 17.2 ms CPIs).
Every CPI the receiver produces a noisy range/angle measurement whose
variance is set by the post-CPI SNR (consistent with the Cramer-Rao
bounds derived in the paper). A first-order alpha-smoother closed-loop
tracker then:

    1. predicts the target position one CPI ahead,
    2. blends the prediction with the new measurement (gain alpha),
    3. steers the R-RIS beam to the predicted angle, and
    4. evaluates the realised beam gain at the true target angle.

Five live panels show the bistatic scene + steered beam, the R-RIS array
factor, a magnified tracking view, the angle-tracking time series, and the
position-error history with running RMSE.

INTERACT
--------
  * Speed   slider : target speed              (0.5 - 6 m/s)
  * Heading slider : target heading            (0 - 90 deg)
  * Alpha   slider : smoother gain             (0.05 - 1.0)
  * SNR     slider : post-CPI SNR -> noise     (0 - 30 dB)
  * Play / Pause   : start / stop the animation
  * Reset          : restart the run from the paper's initial condition
  * Smoother box   : toggle the alpha-smoother on/off (see raw vs filtered)

You can change the speed/heading WHILE it runs to "drive" the target and
watch the beam chase it.

REQUIREMENTS
------------
    pip install numpy matplotlib
Run from a normal desktop Python (needs an interactive matplotlib backend,
e.g. TkAgg/Qt). Simply:  python ris_mfsk_tracking_demo.py
=========================================================================
"""
...
# ------------------------------------------------------------------ #
#  1.  PHYSICAL CONSTANTS AND SYSTEM PARAMETERS (from the paper)      #
# ------------------------------------------------------------------ #
...
COL_TRUE = '#1f6feb'    # true target
COL_EST  = '#d29922'    # estimate
COL_CMD  = '#8957e5'    # beam command
COL_BEAM = '#a371f7'    # beam cone
COL_GOOD = '#2da44e'
COL_BAD  = '#cf222e'
