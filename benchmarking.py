"""
benchmarking.py  —  Stage 6b: System-Level Benchmarking
=========================================================
Quantitative comparison of four sensing/radar architectures across power
consumption, waveform agility, range resolution, beam agility, and cost.

Architectures
-------------
  1. Two-stage T-RIS/R-RIS (proposed)  — DDS-limited at 714 kHz
  2. Single-stage STM-RIS (baseline)   — SPI-limited at  13 kHz
  3. Conventional FMCW radar           — chirp-based, 28 GHz
  4. AESA phased-array radar           — active electronically-scanned

Power calibration (two-stage)
-------------------------------
  PA input (20 % eff., PTx = 100 mW)     :  0.500 W
  T-RIS PIN-diode DC bias (N × 2 mW/el.) :  0.512 W
  R-RIS PIN-diode DC bias (N × 2 mW/el.) :  0.512 W
  DDS dynamic switching N·Esw·M·fSym     :  0.022 W
  DSP + ADC                              :  0.550 W
  Rx LNA chain                           :  0.080 W
  Controller / clock                     :  0.150 W
  ─────────────────────────────────────────────────
  Total                                  :  2.326 W  ≈ 2.33 W  ✓

Output figures
--------------
F17 — Stacked horizontal bar chart of power breakdown (all 4 architectures)
      Single-column IEEE TVT  (3.5 × 2.6 in)
F18 — Spider / radar chart of 5 normalised performance metrics × 4 architectures
      Single-column IEEE TVT  (3.5 × 3.0 in)

Output tables  [all critical for paper]
-----------------------------------------
T4  — Two-stage vs single-stage quantitative comparison  ★
T5  — Prior RIS sensing literature comparison            ★
T6  — Computational complexity comparison
T8  — Detailed power budget breakdown

Output data
-----------
data/benchmarking_data.npy
...
def define_architectures() -> list:
    ...
    Power keys are unified across architectures so the stacked bar chart
    has a consistent legend.
    """
    N  = cfg.N_ELEM
    c  = cfg.C_LIGHT
    M  = cfg.M_TONES
...
def run(show: bool = False):
    ...
    if show:
        plt.show()
    plt.close('all')
    print('\n  Stage 6b complete — 2 figures + 4 tables + data saved.')
    return archs, norm
