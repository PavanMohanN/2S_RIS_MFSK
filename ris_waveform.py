...
=================================================================
Simulates the M-FSK waveform generated by the T-RIS via DDS switching
and characterises the beam squint across the M-tone bandwidth.

Paper equations:
    s_n(t) = Tmean·cos(2πfc·t)
            + Tsb/2·cos[2π(fc+fm,k)·t]      ← desired upper sideband
            + Tsb/2·cos[2π(fc-fm,k)·t]      ← image lower sideband  [Eq. 7]
    Δθ_squint ≈ arcsin(sinθ1 · fm,k / fc)   ← beam squint           [Eq. 14]

Output figures
--------------
F7 — M-FSK spectrum: 12 upper tones, 12 image tones, BPF overlay,
     carrier feedthrough.  Single-column IEEE TVT.
F8 — Time-frequency spectrogram over 2 full sweeps.
     Single-column IEEE TVT.

Output data
-----------
data/mfsk_waveform_data.npy — spectrum dict for downstream stages

Usage
-----
    python ris_waveform.py          # standalone
...
# ─────────────────────────────────────────────────────────────────────────────
# IEEE TVT publication style (shared settings)
# ─────────────────────────────────────────────────────────────────────────────
plt.rcParams.update({
    'font.family':          'serif',
